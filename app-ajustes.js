...
function escucharYListarProfesores() {
  ...
  onValue(profesoresRef, (snapshot) => {
    ...
    if (!profesores) {
      ...
      return; // El return correcto va dentro del "if" únicamente
    }
    ...
    Object.keys(profesores).forEach(key => {
      ...
      // Creamos el diseño de la tarjeta integrando el switch/checkbox
      const itemHTML = `
        <div class="flex items-center justify-between p-3 bg-gray-50 border rounded-xl hover:shadow-sm transition-all mb-2">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <!-- Checkbox para activar/desactivar el reporte por correo -->
      ...
      // Añadimos el evento interactivo al checkbox recién creado
      ...
      if (checkReporte) {
        checkReporte.addEventListener('change', async (e) => {
          ...
          try {
            ...
            e.target.checked = !nuevoEstado; // Revierte el check si hay un error de conexión
          }
        });
      }
    });
